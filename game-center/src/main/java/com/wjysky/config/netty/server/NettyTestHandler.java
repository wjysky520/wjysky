...
public class NettyTestHandler extends SimpleChannelInboundHandler<ByteBuf> {
    ...
    private static NettyTestHandler testHandler = new NettyTestHandler();

    private NettyTestHandler() {}

    public static NettyTestHandler getInstance() {
        return testHandler;
    }
...
//			ctx.channel().writeAndFlush(data);
            ...
            log.debug("Netty测试服务器向客户端发送信息成功");
            return true;
        } catch (Exception e) {
    ...
    public void userEventTriggered(ChannelHandlerContext ctx, Object evt) throws Exception {
        if (evt instanceof IdleStateEvent){
            ...
            if (idleStateEvent.state().equals(IdleState.READER_IDLE)){ // 长期未收到客户器发送的信息
                log.info("Netty测试服务器已经30秒没有收到客户端的信息！");
                // 向客户端发送消息
//                ctx.channel().writeAndFlush("hi\r\n".getBytes(CharsetUtil.UTF_8)).addListener(ChannelFutureListener.CLOSE_ON_FAILURE);
                ...
                log.info("Netty测试服务器已经30秒没有发送信息给客户端！");
